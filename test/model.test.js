...
test("resolveMode prefers drag, then fall/stun/sick, moods, care, media, and sneak", () => {
  ...
  assert.equal(Model.resolveMode({ wander: "walk", mediaPlaying: true, video: true }), "watch")
...
test("pickWander mixes walk, run, idle, and look", () => {
  ...
  assert.equal(Model.pickWander(0.33), "walk")
  assert.equal(Model.pickWander(0.34), "run")
  assert.equal(Model.pickWander(0.63), "run")
  assert.equal(Model.pickWander(0.64), "idle")
...
test("stepFall accelerates until the floor", () => {
  ...
  assert.equal(Model.isMoveMode("run"), true)
})
...
test("generatedAtlas builds the 16x12 overlay spec", () => {
  ...
  const fall = Model.framesForMode(spec, "fall")
  assert.equal(fall.frameY, 560)
  assert.equal(fall.frameCount, 16)
